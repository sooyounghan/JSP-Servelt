...
-----
### 예약 정보 삭제를 위한 CarReserveView의 Delete 연결
-----
```jsp
...
...
...
```
  * location.href='RentCarMain.jsp?center=CarReserveDelete.jsp?id=<%=id%>&reserve_day=<%=car.getReserve_day()%>으로 설정할 경우,
    + 해당 RentCarMain에서 지속적으로 쿼리문자열 정보를 남기게 되므로, 추후 삭제를 하더라도 RentCarMain.jsp에서 center=null이 되지 못하므로 삭제에 대한 처리는 위와 같이 처리

-----
### CarReserveDelete.jsp
-----
```jsp
...
-----
### RentCarDAO : reserveCarRemove 구현
-----
```java
package RentCar;
